...
for ($i = 1; $i -le $Iterations; $i++) {
    $body = @{ question = $Question; topK = $TopK } | ConvertTo-Json
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    try {
        $resp = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/repos/$RepoId/chat/answer" -Headers $headers -ContentType "application/json" -Body $body -TimeoutSec 45
        $sw.Stop()
...
Write-Host "== benchmark_chat (Mock LLM) =="
